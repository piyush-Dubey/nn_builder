...
class CNN(nn.Module, Base_Network):
    ...
    def check_CNN_layers_valid(self):
        ...
        for layer in all_layers:
            ...
            if layer_type_name == "conv":
                ...
                for ix in range(2):
                    assert isinstance(layer[ix + 1], int) and layer[ix + 1] > 0, error_msg_maxpool_layer
                if layer[1] != layer[2]:
                    print("NOTE that your maxpool kernel size {} isn't the same as your stride {}".format(
                        layer[1], layer[2]))
                assert isinstance(layer[3], int) and layer[3] >= 0, error_msg_maxpool_layer
            elif layer_type_name == "maxunpool":
                assert len(layer) == 4, error_msg_maxunpool_layer
                for ix in range(2):
                    assert(layer[ix + 1], int) and layer[ix + 1] > 0, error_msg_maxunpool_layer
                if layer[1] != layer[2]:
                    print("NOTE that your maxunpool kernel size {} isn't the same as your stride {}".format(
                        layer[1], layer[2]))
                assert isinstance(layer[3], int) and layer[3] >= 0, error_msg_maxunpool_layer
            elif layer_type_name == "avgpool":
                assert len(layer) == 4, error_msg_avgpool_layer
                for ix in range(2):
                    assert isinstance(layer[ix + 1], int) and layer[ix + 1] > 0, error_msg_avgpool_layer
                assert isinstance(layer[3], int) and layer[3] >= 0, error_msg_conv_layer
                if layer[1] != layer[2]:
                    print("NOTE that your avgpool kernel size {} isn't the same as your stride {}".format(
                        layer[1], layer[2]))
            elif layer_type_name == "adaptivemaxpool":
                assert len(layer) == 3, error_msg_adaptivemaxpool_layer
                for ix in range(2):
                    assert isinstance(layer[ix + 1], int) and layer[ix + 1] > 0, error_msg_adaptivemaxpool_layer
            elif layer_type_name == "adaptiveavgpool":
                assert len(layer) == 3, error_msg_adaptiveavgpool_layer
                for ix in range(2):
                    assert isinstance(layer[ix + 1], int) and layer[ix + 1] > 0, error_msg_adaptiveavgpool_layer
            elif layer_type_name == "linear":
                assert len(layer) == 2, error_msg_linear_layer
                for ix in range(1):
                    assert isinstance(layer[ix+1], int) and layer[ix+1] > 0
            else:
                raise ValueError("Invalid layer name")

        rest_must_be_linear = False
        for ix, layer in enumerate(all_layers):
            if rest_must_be_linear:
                assert layer[0].lower() == "linear", "If have linear layers then they must come at end"
    ...
    def create_hidden_layers(self):
        ...
        for layer in self.layers_info[:-1]:
            input_dim = self.create_and_append_layer(
                input_dim, layer, cnn_hidden_layers)
    ...
    def create_and_append_layer(self, input_dim, layer, list_to_append_layer_to):
        ...
        elif layer_name == "maxunpool":
            list_to_append_layer_to.extend(
                [nn.MaxUnpool2d(kernel_size=layer[1], stride=layer[2], padding=layer[3])])
        elif layer_name == "avgpool":
            list_to_append_layer_to.extend([nn.AvgPool2d(kernel_size=layer[1],
                                                         stride=layer[2], padding=layer[3])])
        elif layer_name == "adaptivemaxpool":
            list_to_append_layer_to.extend(
                [nn.AdaptiveMaxPool2d(output_size=(layer[1], layer[2]))])
        elif layer_name == "adaptiveavgpool":
            list_to_append_layer_to.extend(
                [nn.AdaptiveAvgPool2d(output_size=(layer[1], layer[2]))])
        elif layer_name == "linear":
            if isinstance(input_dim, tuple):
                input_dim = np.prod(np.array(input_dim))
            list_to_append_layer_to.extend(
                [nn.Linear(in_features=input_dim, out_features=layer[1])])
        else:
    ...
    def calculate_new_dimensions(self, input_dim, layer):
        ...
        if layer_name == "conv":
            ...
            new_channels = input_dim[0]
            kernel, stride, padding = layer[1], layer[2], layer[3]
            new_height = int((input_dim[1] - 1) * stride - 2 * padding + kernel)
            new_width = int((input_dim[2] - 1) * stride - 2 * padding + kernel)
            output_dim = (new_channels, new_height, new_width)
    ...
    def create_output_layers(self):
        ...
        if not isinstance(self.layers_info[-1][0], list):
            self.layers_info[-1] = [self.layers_info[-1]]
        for output_layer in self.layers_info[-1]:
            self.create_and_append_layer(
                input_dim, output_layer, output_layers)
        return output_layers

    def initialise_all_parameters(self):
        """Initialises the parameters in the linear and embedding layers"""
        initialisable_layers = [layer for layer in self.hidden_layers if not type(
            layer) in self.valid_layer_types_with_no_parameters]
    ...
    def create_batch_norm_layers(self):
        ...
        for layer in self.layers_info[:-1]:
            ...
            if layer_type == "conv":
                batch_norm_layers.extend(
                    [nn.BatchNorm2d(num_features=layer[1])])
            elif layer_type == "linear":
                batch_norm_layers.extend(
                    [nn.BatchNorm1d(num_features=layer[1])])
        return batch_norm_layers

    def forward(self, x):
        """Forward pass for the network. Note that it expects input data in the form (Batch, Channels, Height, Width)"""
        if not self.checked_forward_input_data_once:
            self.check_input_data_into_forward_once(x)
    ...
    def check_input_data_into_forward_once(self, x):
        ...
        assert x.shape[1:] == self.input_dim, "Input data must be of shape (channels, height, width) that you provided, not of shape {}".format(
            x.shape[1:])
        ...
        for layer_ix, layer in enumerate(self.hidden_layers):
            if type(layer) in self.valid_layer_types_with_no_parameters:
                ...
                if type(layer) == nn.Linear and not flattened:
                    ...
                    flattened = True
                x = self.get_activation(
                    self.hidden_activations, layer_ix)(layer(x))
                if self.batch_norm:
    ...
    def process_output_layers(self, x):
        ...
        for output_layer_ix, output_layer in enumerate(self.output_layers):
            activation = self.get_activation(
                self.output_activation, output_layer_ix)
